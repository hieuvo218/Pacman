...
class CornersProblem(search.SearchProblem):
    ...
    def __init__(self, startingGameState):
        ...
        "*** YOUR CODE HERE ***"
        coordination = startingGameState.getPacmanPosition()
        self.startState = (coordination, (0, 0, 0, 0))
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not self.walls[nextx][nexty]:
                ...
                if (nextx, nexty) == self.corners[0]:
                    nextState = ((nextx, nexty), (1, corners[1], corners[2], corners[3]))
                elif (nextx, nexty) == self.corners[1]:
                    nextState = ((nextx, nexty), (corners[0], 1, corners[2], corners[3]))
                elif (nextx, nexty) == self.corners[2]:
                    nextState = ((nextx, nexty), (corners[0], corners[1], 1, corners[3]))
                elif (nextx, nexty) == self.corners[3]:
                    nextState = ((nextx, nexty), (corners[0], corners[1], corners[2], 1))
                else:
...
def cornersHeuristic(state, problem):
    ...
    for i in range(4):
        if state[1][i] == 0:
            xy1 = state[0]
            xy2 = corners[i]
            h = abs(xy1[0] - xy2[0]) + abs(xy1[1] - xy2[1])
            if max < h:
                max = h
    return max
